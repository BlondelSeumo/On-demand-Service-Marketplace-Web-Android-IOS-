...
//                $currency_rate = $credit - $debit;
                $currency_rate = get_gigs_currency($user_wallet['wallet_amt'], $user_info->currency_code, $currrency);
                ...
                if (!empty($user_post_data['from_time']) && !empty($user_post_data['to_time']) && !empty($user_post_data['service_date']) && !empty($user_post_data['service_id']) && !empty($user_post_data['latitude']) && !empty($user_post_data['longitude']) && !empty('location') && !empty($user_post_data['notes']) && !empty($user_post_data['amount'])) {
                    ...
                    foreach ($new_timingarray as $booked_time) {
                        ...
                        if ($booked_time['start_time'] == $from_time_railwayhrs && $booked_time['end_time'] == $to_time_railwayhrs) {
                            ...
                            $charges_array = array();
                            $amount = (!empty($UserCurrency)) ? get_gigs_currency($user_post_data['amount'], $booked_time['currency_code'], $UserCurrency) : $user_post_data['amount'];
                            ...
                            $user_post_data['currency_code'] = (!empty($UserCurrency)) ? currency_code_sign($UserCurrency) : currency_code_sign(settings('currency'));
            ...
            if (!empty($user_data['text']) && !empty($user_data['latitude']) && !empty($user_data['longitude'])) {
                ...
                if (is_array($result) && !empty($result)) {
                    ...
                    foreach ($result as $details) {
                        ...
                        $res['service_amount'] = (!empty($UserCurrency && $details['currency_code'] != '')) ? get_gigs_currency($details['service_amount'], $details['currency_code'], $UserCurrency) : $details['service_amount'];
                ...
                if ($result) {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $details) {
                            ...
                            $res['service_title'] = $details['service_title'];
                            $res['service_amount'] = (!empty($ProviderCurrency) && $ProviderCurrency != '') ? get_gigs_currency($details['service_amount'], $details['currency_code'], $ProviderCurrency) : $details['service_amount'];
                            $res['category_name'] = $details['category_name'];
                            $res['subcategory_name'] = $details['subcategory_name'];
                ...
                if ($result) {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $details) {
                            ...
                            $res['service_title'] = $details['service_title'];
                            $res['service_amount'] = (!empty($UserCurrency)) ? get_gigs_currency($details['service_amount'], $details['currency_code'], $UserCurrency) : $details['service_amount'];
                            $res['category_name'] = $details['category_name'];
                            $res['subcategory_name'] = $details['subcategory_name'];
...
//                            print_r($result);exit;
                            ...
                            $services['service_amount'] = (!empty($ProviderCurrency) && $ProviderCurrency != '') ? get_gigs_currency($result['service_amount'], $result['currency_code'], $ProviderCurrency) : $result['service_amount'];
                            ;
                ...
                if ($result) {
                    ...
                    if (!empty($details)) {
                        ...
                        foreach ($details as $result) {
                            ...
                            $services['service_id'] = $result['service_id'];
                            $services['service_title'] = $result['service_title'];
                            $services['service_amount'] = (!empty($UserCurrency)) ? get_gigs_currency($result['service_amount'], $result['currency_code'], $UserCurrency) : $result['service_amount'];
                            ;
                            $services['about'] = $result['about'];
                            $services['service_offered'] = $result['service_offered'];
                            $services['category_name'] = $result['category_name'];
            ...
            if (!empty($user_data['latitude']) && !empty($user_data['longitude']) && !empty($user_data['subcategory_id'])) {
                ...
                if (!empty($val)) {
                    ...
                    foreach ($val as $key => $value) {
                        ...
                        $data[$key]['service_amount'] = (!empty($UserCurrency) && $value['currency_code'] != '') ? get_gigs_currency($value['service_amount'], $value['currency_code'], $UserCurrency) : $value['service_amount'];
                ...
                if ($is_available_user == 0) {
                    ...
                    if ($is_available_mobile == 0 && $is_available_mobileno == 0) {
                        ...
                        if (!empty($user_data['name']) && !empty($user_data['email']) && !empty($user_data['mobileno']) && !empty($user_data['category']) && !empty($user_data['subcategory']) && !empty($user_data['country_code']) && !empty($user_data['device_type'])) {
                            ...
                            $user_details['currency_code'] = 'USD';
        ...
        if (!empty($token)) {
            ...
            if (!empty($history)) {
                ...
                foreach ($history as $key => $value) {
                    ...
                    $his[$key]['currency'] = currency_code_sign(settings('currency'));
...
//                                    print_r($amount);exit;
    ...
    public function paypal_success($amt, $orderId, $transaction_id, $token_id) {
        ...
        if ($this->db->insert('wallet_transaction_history', $history_pay)) {
            /* update wallet table */
            $wallet_amt = $curren_wallet + $history_pay['credit_wallet'];
