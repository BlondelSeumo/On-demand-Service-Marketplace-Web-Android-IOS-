...
public class DialogEditCategoryListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    AlertDialog dialog;
    EditText tvCategory, etSubCategory;
    public boolean isAvaialble = true;
    Button btnDone;


    public DialogEditCategoryListAdapter(Context mContext, List<ServiceCategories.CategoryList> category_list, AlertDialog dialog, EditText etCategory, EditText etSubCategory, String cat_ID, String subCatID, Button btnDone) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        categoryViewHolder.tvCatName.setText(category_list.get(position).getCategoryName());
        ...
        categoryViewHolder.tvCatName.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String catName = "";
                catName = category_list.get(position).getCategoryName();

                if (!category_list.get(position).getId().equalsIgnoreCase(cat_id)) {
                    cat_id = category_list.get(position).getId();
                    if (!cat_id.isEmpty()) {
                        subCatID = "";
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < category_list.size(); i++) {
                    if (category_list.get(i).isChecked()) {
                        catName = category_list.get(i).getCategoryName();

                        cat_id = category_list.get(i).getId();
                    }
                }
                if (!cat_id.isEmpty()) {
                    tvCategory.setText(catName);
                    ...
                    Toast.makeText(mContext, "Select any one category to proceed", Toast.LENGTH_SHORT).show();
                }
