...
import com.dreamguys.truelysell.TabBar.animate.Animatable;
import com.dreamguys.truelysell.TabBar.animate.AnimationType;
import com.dreamguys.truelysell.TabBar.animate.FlipAnimater;
import com.dreamguys.truelysell.TabBar.animate.JumpAnimater;
import com.dreamguys.truelysell.TabBar.animate.RotateAnimater;
import com.dreamguys.truelysell.TabBar.animate.Scale2Animater;
import com.dreamguys.truelysell.TabBar.animate.ScaleAnimater;
import com.dreamguys.truelysell.TabBar.anno.NorIcons;
import com.dreamguys.truelysell.TabBar.anno.SeleIcons;
import com.dreamguys.truelysell.TabBar.anno.Titles;
 ...
 */
public class CustomTabBar extends LinearLayout implements ViewPager.OnPageChangeListener, View.OnTouchListener {
    //默认的图标大小
    private static final int DEFAULT_ICONSIZE = 24;
    //字体默认大小
    private static final int DEFAULT_TEXTSIZE = 12;
    //默认的没有选中的文字和图标的颜色
    private static final int DEFAULT_NORMAL_COLOR = 0xFF0080;
    //默认的上下背景间隔
    private static final int DEFAULT_MARGIN = 8;
    //默认的选中颜色
    private static final int DEFAULT_SELECT_COLOR = 0xFF0080;
    //默认是否接受颜色随着字体变化
    ...
    //所有Tabitem
    private CustomTabItem[] mCustomTabItems;
    // 中间按钮
    ...
    public CustomTabBar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void initFromAttribute() {
        ...
        if (!isInEditMode()) {
            ...
            //计算Tab的宽度
            mCustomTabItems = new CustomTabItem[mNormalIcons.length];
            //实例化TabItem添加进去
            for (int i = 0; i < mCustomTabItems.length; i++) {
                final int temp = i;
                ...
                mCustomTabItems[i] = new CustomTabItem.Builder(mContext).setTitle(mTitles == null ? null : mTitles[i]).setIndex(temp).setTextSize(textSize)
                ...
                mCustomTabItems[i].setTag(temp);
                mCustomTabItems[i].setOnTouchListener(this);
                addView(mCustomTabItems[i]);
                //判断是不是准备到中间的tab,假如设置了中间图标就添加进去
                if (i == (mCustomTabItems.length / 2 - 1) && mAttribute.getResourceId(R.styleable.JPTabBar_TabMiddleView, 0) != 0) {
                    //添加中间的占位距离控件
            ...
            for (int i = 1; i < mCustomTabItems.length; i++) {
                mCustomTabItems[i].setSelect(false, false);
            }
            mCustomTabItems[0].setSelect(true, true, false);
    ...
    private void setSelectTab(int index, boolean animated) {
        if (mCustomTabItems == null || index > mCustomTabItems.length - 1) return;
        mSelectIndex = index;
        for (int i = 0; i < mCustomTabItems.length; i++) {
            if (i == index) {
                continue;
            }
            if (!mCustomTabItems[i].isSelect()) {
                mCustomTabItems[i].setSelect(false, animated);
            } else {
                mCustomTabItems[i].setSelect(false, animated);
            }
        }
        mCustomTabItems[index].setSelect(true, animated);
    ...
    private void updateTitles(String... titles) {
        for (int i = 0; i < mTitles.length; i++) {
            if (!titles[i].equals(mTitles[i])) {
                mCustomTabItems[i].setTitle(titles[i]);
            }
        }
        mTitles = titles;
    }
     ...
     * 获取选中的Tab
     */
    public CustomTabItem getSelectedTab() {
        for (int i = 0; i < mCustomTabItems.length; i++) {
            if (mCustomTabItems[i].isSelect()) {
                return mCustomTabItems[i];
            }
        }
        return null;
    }
     ...
     * 获取第几个Tab
     */
    public CustomTabItem getTabAtPosition(int pos) {
        if (pos < 0 || pos >= mCustomTabItems.length) {
            throw new TabException("invalid position parameter");
        }
        return mCustomTabItems[pos];
    }
    ...
    public int getTabsCount() {
        return mCustomTabItems == null ? 0 : mCustomTabItems.length;
    }
    ...
    public void showBadge(int pos, String text, boolean draggable) {
        if (mCustomTabItems != null) {
            mCustomTabItems[pos].showTextBadge(text);
            mCustomTabItems[pos].getBadgeViewHelper().setDragable(draggable);
        }
    }
    ...
    public void showCircleBadge(int pos, boolean draggable) {
        ...
        if (mCustomTabItems != null) {
            mCustomTabItems[pos].showCirclePointBadge();
            mCustomTabItems[pos].getBadgeViewHelper().setDragable(draggable);
        }
    }
    ...
    public void hideBadge(int position) {
        if (position < 0 || position >= mCustomTabItems.length) {
            throw new TabException("invalid position parameter");
        }
        if (mCustomTabItems != null)
            mCustomTabItems[position].hiddenBadge();
    }
     ...
     * @param titles
     */
    public CustomTabBar setTitles(String... titles) {
        if (mTitles == null) {
            this.mTitles = titles;
    ...
    public CustomTabBar setTitles(int... titles) {
    ...
    public void setTitle(int pos, String title) {
        if (pos >= 0 && pos < mCustomTabItems.length) {
            mCustomTabItems[pos].setTitle(title);
        }
    }
     ...
     * 设置为选中的图标数组
     */
    public CustomTabBar setNormalIcons(int... normalIcons) {
        if (mNormalIcons == null) {
            ...
            for (int i = 0; i < mNormalIcons.length; i++) {
                mCustomTabItems[i].setNormalIcon(normalIcons[i]);
            }
    ...
    public void setNormalIcon(int pos, int normalIcon) {
        if (pos >= 0 && pos < mCustomTabItems.length) {
            mCustomTabItems[pos].setNormalIcon(normalIcon);
        }
    }
     ...
     * 设置选中图标
     */
    public CustomTabBar setSelectedIcons(int... selectedIcons) {
        if (mSelectedIcons == null) {
            ...
            for (int i = 0; i < mSelectedIcons.length; i++) {
                mCustomTabItems[i].setSelectIcon(selectedIcons[i]);
            }
    ...
    public void setSelectedIcon(int pos, int selectedIcon) {
        if (pos >= 0 && pos < mCustomTabItems.length) {
            mCustomTabItems[pos].setSelectIcon(selectedIcon);
        }
    }
    ...
    public void setTabTypeFace(Typeface typeFace) {
        for (CustomTabItem tabItem : mCustomTabItems) {
    ...
    public boolean isBadgeShow(int index) {
        if (mCustomTabItems != null)
            return mCustomTabItems[index].isBadgeShow();
        return false;
    }
    ...
    public void setIconSize(int size) {
        if (mCustomTabItems != null) {
            for (CustomTabItem item : mCustomTabItems) {
    ...
    public void setAnimation(AnimationType animationType) {
        for (int i = 0; i < mCustomTabItems.length; i++) {
            mCustomTabItems[i].setAnimater(animationType == AnimationType.SCALE ? new ScaleAnimater() : animationType == AnimationType.ROTATE ? new RotateAnimater() :
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if (mCustomTabItems == null || position > mCustomTabItems.length - 1 || 1 + position > mCustomTabItems.length - 1) return;
        if (positionOffset > 0f) {
            if (mGradientEnable) {
                mCustomTabItems[position].changeAlpha(1 - positionOffset);
                mCustomTabItems[position + 1].changeAlpha(positionOffset);
            }

            if (mCustomTabItems[position].getAnimater() != null && mPageAnimateEnable) {
                if (mCustomTabItems[position].getAnimater().isNeedPageAnimate()) {
                    mNeedAnimate = false;
                    mCustomTabItems[position].getAnimater().onPageAnimate(mCustomTabItems[position].getIconView(), 1 - positionOffset);
                    mCustomTabItems[position + 1].getAnimater().onPageAnimate(mCustomTabItems[position + 1].getIconView(), positionOffset);
                } else {
                    mNeedAnimate = true;
                }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        CustomTabItem tabItem = (CustomTabItem) v;
        if (tabItem.isSelect()) {
            return false;
        }
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mDragedBadge && mCustomTabItems[mSelectIndex].getAnimater() != null&&mPressAnimateEnable) {
                    mCustomTabItems[mSelectIndex].getAnimater().onPressDown(mCustomTabItems[mSelectIndex].getIconView(), true);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!isInRect(v, event) || (mTabSelectLis != null && mTabSelectLis.onInterruptSelect(temp))) {
                    if (mCustomTabItems[mSelectIndex].getAnimater() != null&&mPressAnimateEnable) {
                        mCustomTabItems[mSelectIndex].getAnimater().onTouchOut(mCustomTabItems[mSelectIndex].getIconView(), true);
                        tabItem.getAnimater().onTouchOut((tabItem.getIconView()), false);
                    }

                } else {
                    if (mTabPager != null && mTabPager.getAdapter() != null && mTabPager.getAdapter().getCount() >= mCustomTabItems.length) {
                        mNeedAnimate = true;
                        mTabPager.setCurrentItem(temp, false);
                    } else if (mTabPager != null && mTabPager.getAdapter() != null && mTabPager.getAdapter().getCount() <= mCustomTabItems.length) {
                        mNeedAnimate = true;
                        mTabPager.setCurrentItem(temp, false);
                        setSelectTab(temp);
                    } else {
